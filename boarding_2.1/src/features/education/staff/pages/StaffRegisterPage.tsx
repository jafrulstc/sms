...
import {
  ...
  selectStaffLoading, // Changed from selectTeacherLoading
  selectStaffError, // Changed from selectTeacherError
} from '~/features/education/staff/store/staffSlice'; // Changed path and slice name
import { staffSchema, type StaffFormData } from '../schemas/staffSchema'; // Changed from teacherSchema, TeacherFormData
...
const TabPanel = ({ children, value, index, ...other }: TabPanelProps) => {
  return (
    <div
      role="tabpanel"
      hidden={value !== index}
      id={`staff-tabpanel-${index}`} // Changed from teacher-tabpanel
      aria-labelledby={`staff-tab-${index}`} // Changed from teacher-tab
      {...other}
    >
...
/**
 * Staff Registration Form Component // Changed from Teacher Registration Form Component
 */
const StaffRegisterPage = memo(() => { // Changed from TeacherRegisterPage
  ...
  const loading = useAppSelector(selectStaffLoading); // Changed from selectTeacherLoading
  const error = useAppSelector(selectStaffError); // Changed from selectTeacherError
   ...
   */
  const onSubmit = async (data: StaffFormData) => { // Changed from TeacherFormData
    try {
      await dispatch(createStaff(data)).unwrap(); // Changed from createTeacher
      showToast(`Staff ${SUCCESS_MESSAGES.CREATED}`, 'success'); // Changed from Teacher
      navigate('/admin/education/staff/manage'); // Changed from teachers/manage
    } catch (error: any) {
      showToast(error.message || 'Failed to create staff', 'error'); // Changed from teacher
    }
  };
  ...
  const a11yProps = (index: number) => {
    return {
      id: `staff-tab-${index}`, // Changed from teacher-tab
      'aria-controls': `staff-tabpanel-${index}`, // Changed from teacher-tabpanel
    };
  };
    ...
    <Box>
      ...
      <Box sx={{ mb: 4 }}>
        ...
        <Typography variant="body1" color="text.secondary">
          Register new staff with comprehensive information {/* Changed from Register a new teacher */}
        </Typography>
      </Box>
      ...
      <Paper sx={{ width: '100%' }}>
        ...
        <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
          <Tabs
            ...
            aria-label="staff registration tabs" // Changed from teacher registration tabs
            variant="scrollable"
            scrollButtons="auto"
          >
                        ...
                        <Controller
                          ...
                          render={({ field }) => (
                            <TextField
                              ...
                              multiline
                              rows={4}
                              placeholder="Any additional information about the staff..." // Changed from teacher
              ...
              <Box sx={{ display: 'flex', gap: 2 }}>
                ...
                {activeTab < 4 ? (
                  ...
                  <Button
                    ...
                    sx={{ minWidth: 120 }}
                  >
                    {loading || isSubmitting ? 'Saving...' : 'Save Staff'} {/* Changed from Save Teacher */}
                  </Button>
                )}
              </Box>
            </Box>
...
StaffRegisterPage.displayName = 'StaffManagementPage'; // Changed from TeacherManagementPage

export { StaffRegisterPage }; // Changed from TeacherRegisterPage