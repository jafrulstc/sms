...
} from '~/features/education/teachers/schemas/teacherSchema';
import {
  Teacher,
  CreateTeacherDto,
  UpdateTeacherDto,
} from '~/features/education/teachers/types/teacherType';
...
const TabPanel = ({ children, value, index, ...other }: TabPanelProps) => (
  <div
    role="tabpanel"
    hidden={value !== index}
    id={`teacher-edit-tabpanel-${index}`}
    aria-labelledby={`teacher-edit-tab-${index}`}
    {...other}
  >
...
const TeacherFormModal = memo(({ open, onClose, teacher }: TeacherEditModalProps) => {
  ...
  const a11yProps = (index: number) => ({
    id: `teacher-edit-tab-${index}`,
    'aria-controls': `teacher-edit-tabpanel-${index}`,
  });
  ...
  /* initialise form on modal open / teacher change */
  useEffect(() => {
    if (!open) return;

    if (teacher) {
      const { birthRegNumber, ...restTeacher } = teacher as any; // old typo guard
      const mapped: TeacherFormData = {
        ...restTeacher,
        birthRegNumber: teacher.birthRegNumber ?? teacher.birthRegNumber ?? '',
  ...
  const onSubmit = async (formData: TeacherFormData) => {
    try {
      if (teacher) {
        /* EDIT mode */
        const diff = getChangedFields<TeacherFormData>(teacher as unknown as TeacherFormData, formData);
        ...
        await dispatch(
          updateTeacher({ id: teacher.id, data: diff as UpdateTeacherDto }),
        ).unwrap();
        showToast(`Teacher ${SUCCESS_MESSAGES.UPDATED}`, 'success');
      } else {
        /* CREATE mode */
        await dispatch(createTeacher(formData as CreateTeacherDto)).unwrap();
    ...
    <Dialog open={open} onClose={onClose} maxWidth="lg" fullWidth disableRestoreFocus>
      <DialogTitle sx={{ p: 2 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Typography variant="h6">{teacher ? 'Edit Teacher' : 'Add Teacher'}</Typography>
      ...
      <form onSubmit={handleSubmit(onSubmit)} noValidate>
        ...
        <DialogActions sx={{ p: 2 }}>
          ...
          >
            {loading || isSubmitting ? t('common.loading') : teacher ? t('common.save') : t('common.add')}
          </Button>
        </DialogActions>
      </form>
    </Dialog>
